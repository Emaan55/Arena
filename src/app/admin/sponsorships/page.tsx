...
import type { Sponsorship, ProductSearchResult } from "@/types/database";
import type { SponsorDuration } from "@/lib/sponsorship-constants";
import { SPONSOR_DURATIONS, SPONSOR_PRICE_LABELS } from "@/lib/sponsorship-constants";

const SECRET_STORAGE_KEY = "arena_admin_secret";

type SponsorshipWithProduct = Sponsorship & { product: { id: string; name: string; category: string } };
 ...
 * sessionStorage, sent as `x-admin-secret` on every request below.
 */
export default function AdminSponsorshipsPage() {
  ...
  async function addSponsor() {
    if (!secret || !pickedProductId) return;
    setBusy(true);
    setError(null);
    try {
      const res = await fetch("/api/admin/sponsorships", {
        ...
        body: JSON.stringify({ productId: pickedProductId, durationDays: duration }),
      });
      const json = await res.json();
      if (!res.ok) {
  ...
  return (
    <main className="mx-auto flex max-w-3xl flex-col gap-8 px-6 py-12">
        ...
        {data?.active ? (
          <div className="flex items-center justify-between gap-3 rounded-xl border border-accent bg-accent-soft/5 p-4">
            <div className="flex flex-col gap-0.5">
              <span className="font-semibold text-ink">{data.active.product.name}</span>
              <span className="text-xs text-muted">{data.active.product.category}</span>
        ...
        {!data || data.queue.length === 0 ? (
          ...
          <ul className="flex flex-col gap-2">
            {data.queue.map((s, i) => (
              ...
              >
                <div className="flex flex-col gap-0.5">
                  <span className="text-sm font-semibold text-ink">{s.product.name}</span>
                  <span className="text-xs text-muted">
      ...
      <section className="flex flex-col gap-3 rounded-2xl border border-border bg-surface p-5 shadow-sm">
        <h2 className="font-display text-sm font-bold text-ink">Add a sponsor (free)</h2>
            ...
            {data.history.map((s) => (
              <li key={s.id} className="flex items-center justify-between gap-3 py-2 text-sm">
                <div className="flex flex-col">
                  <span className="text-ink">{s.product.name}</span>
                  <span className="text-xs text-muted">
