...
/**
 * The single callback every Supabase Auth email/OAuth flow redirects
 * back to: signup verification, password recovery, and (still, for any
 * caller that keeps using it) magic-link sign-in all arrive here as
 * `token_hash`+`type` and are verified with verifyOtp() — that IS the
 * email-ownership proof, there's no separate step. X/Twitter OAuth
 * arrives here as a `code` instead (the PKCE flow) and is exchanged with
 * exchangeCodeForSession(). Either way this establishes a real session
 * cookie, then returns the visitor to wherever they were headed (`next`,
...
export async function GET(request: NextRequest) {
  ...
  if (code) {
    ...
    return NextResponse.redirect(new URL("/auth/sign-in?authError=oauth", origin));
  }
